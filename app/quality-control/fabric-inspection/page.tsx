...
export default function FabricInspectionPage() {
    ...
    getFabricInspections().then((res) => {
      ...
      setData({
        metrics: [
          ...
          { label: "Avg. grade", value: items.length ? `${(items.reduce((s: number, i: any) => s + Number(i.grade ?? 0), 0) / items.length).toFixed(1)}/5` : "-", note: "From API", trend: "up" as const },
        ],
  ...
  return <QualityControlWorkspace module="fabric-inspection" metrics={data.metrics} rows={data.rows} isLoading={isLoading} error={error} rawItems={rawItems} />
}
